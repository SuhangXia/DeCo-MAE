...
# 🏆 双雄会师
# Model A: 那个死记硬背但很准的 Baseline
CKPT_BASELINE = "/root/autodl-tmp/checkpoints/sota_v2_best.pth"       
# Model B: 那个懂语义且抗干扰的 SOTA
CKPT_COOLDOWN = "/root/autodl-tmp/checkpoints_cooldown/cooldown_best.pth" 
...
if __name__ == "__main__":
    ...
    print("Load Model A: Baseline (83.6%)...")
    model_a = BaselineModel().cuda().to(torch.bfloat16)
    model_a.load_state_dict(torch.load(CKPT_BASELINE), strict=False)
    model_a.eval()

    print("Load Model B: Cool-down SOTA (85.8%)...")
    model_b = DualHeadMAE(TEXT_PROTOTYPES).cuda().to(torch.bfloat16)
    model_b.load_state_dict(torch.load(CKPT_COOLDOWN), strict=False)
    model_b.eval()

    preds, targets = [], []
    print("🔥 Running Grand Ensemble (A + B)...")
    ...
    with torch.no_grad():
        for x, y in tqdm(dl):
            ...
            # 🔥 融合策略: 
            # 既然 Cool-down 模型(B) 明显更强，我们给它更高的权重 (0.6 vs 0.4)
            final_logits = 0.4 * logits_a + 0.6 * logits_b 
    ...
    print("\n" + "="*40)
    print(f"🏆 GRAND ENSEMBLE RESULT:")
    print(f"Model A (Baseline): 83.60%")
    print(f"Model B (Ours):     85.80%")
